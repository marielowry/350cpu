...
lw $t2, 0($t0)              # load first data value, good
j blguard
blstart:
addi $a0, $zero, 3
jal malloc
...
lw $t2, 0($t0)              # load next input data value, good
add $t1, $zero, $v0         # set curr = new
blguard:
bne $t2, $zero, blstart
add $v0, $t3, $zero         # set $v0 = list head
addi $sp, $sp, -1
lw $ra, 0($sp) # THIS BREAKS CODE, lw followed by jr
jr $ra
...
j siguard
sortiter:
lw $t2, 0($t1)              # $t2 = current.data, good
lw $t3, 0($t6)              # $t3 = current.next.data, good
blt $t2, $t3, sinext
addi $t7, $zero, 1          # $t7 = 1
lw $t4, 1($t1)              # $t4 = current.prev, good
bne $t4, $zero, supprev
j supprevd
supprev:
...
lw $t5, 2($t6)              # $t5 = current.next.next, good
